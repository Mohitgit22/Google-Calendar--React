...
const MainCal3 = () => {
  ...
  const fetchEventsFromAPI = async (max) => {
    try {
  ...
  const getManuallyAddedEvents = () => {
    // Manually define and return additional events
    return [
      {
        summary: 'Manually Added Event',
        description: 'Description of Manually Added Event',
        location: 'Manually Added Location',
        start: { dateTime: new Date().toISOString() },
        end: { dateTime: new Date().toISOString() },
        htmlLink: '#',
      },
  ...
  const loadEvents = async (max = 8) => {
    try {
      const fetchedData = await fetchEventsFromAPI(max);
      const manuallyAddedData = getManuallyAddedEvents();
      const mergedData = [...fetchedData, ...manuallyAddedData];
      const processedEvents = mergedData.map((e) => mapEventObject(e));
